...
class MultiHeadedTargetAttention(nn.Module):
    def __init__(self, n_head, d_model):
        super().__init__()
        self.d_k = d_model // n_head
        self.n_head = n_head

        self.q_proj = nn.Parameter(torch.randn(d_model, n_head, self.d_k))
        self.k_proj = nn.Parameter(torch.randn(d_model, n_head, self.d_k))
        self.v_proj = nn.Parameter(torch.randn(d_model, n_head, self.d_k))

        # self.dropout = nn.Dropout(dropout)

    def forward(self, query, key, value):
        batch_size, seq_len, d_model = key.size(0), key.size(1), key.size(2)

        query = torch.einsum("bnd, dhk->bhnk", query.unsqueeze(1), self.q_proj)
        key = torch.einsum("bnd, dhk->bhnk", key, self.k_proj)
        value = torch.einsum("bnd, dhk->bhnk", value, self.v_proj)
        ...
        p_attn = nn.functional.softmax(attn_scores, dim=-1)
        outputs = torch.matmul(p_attn, value)  # [bs, n_head, seq_len, d_k]
        outputs = outputs.transpose(1, 2).contiguous().view(batch_size, 1, d_model)  # [bs, 1, dim]
