...
class RecDataloader(AbstractDataloader):
    ...
    def _get_eval_dataset(self):
        dataset = RecEvalDataset(self.train, self.train_b, self.val, self.val_b, self.val_num, self.seg_len, self.num_items, self.target_code)
        return dataset
...
class RecEvalDataset(data_utils.Dataset):
    def __init__(self, u2seq, u2b, u2answer, u2ab, val_num, max_len, num_items, target_code):
        self.u2seq = u2seq
        self.u2b = u2b
